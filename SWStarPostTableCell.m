...
@implementation SWStarPostTableCell
...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString*)identifier {
    ...
    [starButton setBackgroundImage:[UIImage imageNamed:@"star-inactive-clear.png"] forState:UIControlStateNormal];
    [starButton setBackgroundImage:[UIImage imageNamed:@"star-active-clear.png"] forState:UIControlStateHighlighted];
    ...
    //Create Star Label
    starLabel = [[UILabel alloc]initWithFrame:CGRectMake(254.0, 6.0, 55.0, 55.0)];
    starLabel.text = @"594★";
    starLabel.font = [UIFont boldSystemFontOfSize:24];
    starLabel.textColor = RGBCOLOR(79, 89, 105);
...
- (void)setObject:(id)object {
    if (_item != object) {
        ...
        if ([currentUser.starredPostIDs containsObject:[NSNumber numberWithInt: item.ID]]) {
            ...
            [starButton setBackgroundImage:[UIImage imageNamed:@"star-active-clear.png"] forState:UIControlStateNormal];
            starLabel.text = [NSString stringWithFormat:@"%d★",item.starCount];
...
- (void)layoutSubviews {
    ...
    //Layout StarLabel
    CGSize starLabelSize = [starLabel.text sizeWithFont:starLabel.font];
    
    CGFloat starLabelLeft = starButton.left - (starLabelSize.width / 2) + 27.0;
    CGFloat starLabelTop = (height / 2) - 28.0;
    
    starLabel.frame = CGRectMake(starLabelLeft, starLabelTop, starLabelSize.width, starLabelSize.height + 7);
